...
\begin{frame}{Conv-SDNN预测模型——CNN预测模型}
    ...
    % 一个具有$C$个卷积核的CNN预测模型可表示为：
    % \begin{align*}
    %     m_j^t & = \sigma \left(\sum^{K_m}_{k_m=1} w^{k_m}_j x_{t+k_m-1} +b_j\right), \quad  & t & = 1,\dots, T-K_m +1,    \\
    %     p_j^i & = \frac{\sum^{K_p}_{k_p=1} m_j^{i+k_p-1}}{K_p}, \quad                       & i & = 1,..., T-K_p - K_m+2, \\
    %     f_C   & = \sum^C_{j=1} \left( \sum^{T-K+2}_{i=1}\beta_{j}^i p_j^i\right) + \beta_0. &   &
    % \end{align*}

    % $K = K_p + K_m$，$K_p$和$K_m$分别为池化操作与卷积操作的宽度大小
